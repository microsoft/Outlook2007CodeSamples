...
/*
 *  IExchExtBgSyncCallback
 *
 *	Purpose:
 *		Background sync: progress and done messages to the client.
 */
#undef INTERFACE
#define INTERFACE   IExchExtBgSyncCallback

DECLARE_INTERFACE_(IExchExtBgSyncCallback, IUnknown)
{
	BEGIN_INTERFACE
	...
	// *** IExchExtBgSyncCallback methods ***
	STDMETHOD(Progress) (THIS_ WCHAR *pwszProgress) PURE;
	STDMETHOD(Done) (THIS_ HANDLE hThread) PURE;
};
typedef IExchExtBgSyncCallback FAR * LPEXCHEXTBGSYNCCALLBACK;

/*
 *  IExchExtRulesSyncCallback
 *
 *	Purpose:
 *		A callback to trigger rules while synching
 */
#undef INTERFACE
#define INTERFACE   IExchExtRulesSyncCallback

DECLARE_INTERFACE_(IExchExtRulesSyncCallback, IUnknown)
{
	BEGIN_INTERFACE
	...
	// *** IExchExtRulesSyncCallback methods ***
	STDMETHOD(TriggerRules) (THIS_ LPMDB pmdbOnline, LPMDB pmdbPubOnline) PURE;
};
typedef IExchExtRulesSyncCallback FAR * LPEXCHEXTRULESSYNCCALLBACK;
#endif // Win32 and not MAC
...
#if defined(WIN32) && !defined(MAC)
DEFINE_EXCHEXTGUID(IID_IExchExtBgSyncCallback,		0x1b);
DEFINE_EXCHEXTGUID(IID_IExchExtRulesSyncCallback,	0x1c);
#endif // Win32 and not Mac
...
#define EECMDID_FileNewDiscFolder					38
